...
class Gap:
    ...
    )  # Optional: From gap analysis report
...
class GapAnalyzer:
    ...
    def __init__(
        self, config: Optional[Dict] = None, completeness_threshold: float = 0.8
    ):
        """
    ...
    def apply_decay_weighting(
        ...
        raw_score = (
            completeness_score / 100.0 if completeness_score > 1 else completeness_score
        )
        ...
        best_paper = max(
            papers, key=lambda p: p.get("estimated_contribution_percent", 0)
        )
        ...
        final_score = (
            1 - self.decay_weight
        ) * raw_score + self.decay_weight * decay_adjusted_score

        # Convert back to 0-100 scale
        final_score_percent = (
            final_score * 100 if completeness_score > 1 else final_score
        )
    ...
    def extract_gaps(
        self, report: Dict, threshold: Optional[float] = None
    ) -> List[Gap]:
        """
                ...
                for sub_req_id, sub_req_data in sub_requirements.items():
                    ...
                    if completeness < threshold:
                        gap = Gap(
                            ...
                            suggested_searches=sub_req_data.get(
                                "suggested_searches", []
                            ),
                        )
                        gaps.append(gap)
    ...
    def generate_gap_summary(self, gaps: List[Gap]) -> Dict:
        ...
        summary["average_completeness"] = sum(
            g.current_completeness for g in gaps
        ) / len(gaps)
    ...
    def generate_decay_impact_report(
        self,
        pillar_name: str,
        requirements: Dict,
        ...
        for req_key, req_data in requirements.items():
            ...
            for sub_req in req_data:
                ...
                impact = (
                    "decreased"
                    if delta < -0.05
                    else "increased" if delta > 0.05 else "minimal"
                )
        ...
        if comparison_report:
            avg_delta = sum(r["delta"] for r in comparison_report) / len(
                comparison_report
            )
            significant_changes = [
                r for r in comparison_report if abs(r["delta"]) > 0.1
            ]
        else:
            avg_delta = 0
            significant_changes = []
        ...
        return {
            ...
            "summary": {
                ...
                "impact_breakdown": {
                    "decreased": len(
                        [r for r in comparison_report if r["impact"] == "decreased"]
                    ),
                    "increased": len(
                        [r for r in comparison_report if r["impact"] == "increased"]
                    ),
                    "minimal": len(
                        [r for r in comparison_report if r["impact"] == "minimal"]
                    ),
                },
            },
        }
...
def extract_gaps_from_file(report_path: str, threshold: float = 0.8) -> List[Gap]:
    """
