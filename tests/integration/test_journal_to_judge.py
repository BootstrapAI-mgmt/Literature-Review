...
class TestJournalToJudgeFlow:
    ...
    @pytest.mark.integration
    def test_version_history_preserves_original_data(
        self, temp_workspace, test_data_generator
    ):
        ...
        original_history["preserve_test.pdf"][0]["review"][
            "CUSTOM_FIELD"
        ] = "test_value"
        original_claim = original_history["preserve_test.pdf"][0]["review"][
            "Requirement(s)"
        ][0]
        ...
        updated_claim = next(
            c
            for c in latest_version["review"]["Requirement(s)"]
    ...
    @pytest.mark.integration
    def test_judge_outputs_multidimensional_scores(
        self, temp_workspace, test_data_generator
    ):
        """
        ...
        claim = version_history["high_quality_paper.pdf"][0]["review"][
            "Requirement(s)"
        ][0]
        ...
        claim["evidence"] = (
            "Strong experimental validation with public code repository."
        )
        ...
        claim = final_history["high_quality_paper.pdf"][-1]["review"]["Requirement(s)"][
            0
        ]
    ...
    @pytest.mark.integration
    def test_judge_rejects_low_quality_evidence(
        self, temp_workspace, test_data_generator
    ):
        """
        ...
        claim = final_history["low_quality_paper.pdf"][-1]["review"]["Requirement(s)"][
            0
        ]
